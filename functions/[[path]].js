/**
 * Cloudflare Pages Catch-All Function: functions/[[path]].js
 * Handles all /api/* requests with D1/KV/R2 and falls back to static assets via context.next()
 */
export async function onRequest(context) {
  ...
  if (url.pathname === '/api/sync') {
    // POST /api/sync - Batch Upsert into D1
    if (request.method === 'POST') {
      try {
        let bodyText = '';
    ...
    // GET /api/sync - Query D1 Database
    try {
      if (env.DB) {
  ...
  // ==================== 4. /api/backup/:filename ====================
  if (url.pathname.startsWith('/api/backup/')) {
    const filename = url.pathname.replace('/api/backup/', '');
    ...
    if (request.method === 'PUT' || request.method === 'POST') {
      try {
        ...
        return new Response(JSON.stringify({ success: true, filename, message: `อัปโหลดไฟล์สำรอง ${filename} สำเร็จ` }), {
          headers: { 'Content-Type': 'application/json', ...corsHeaders }
        });
      } catch (err) {
        return new Response(JSON.stringify({ success: false, error: err.message }), {
  ...
  // Fallback to static asset hosting for index.html, app.js, app.css, images, etc.
  return next();
}
