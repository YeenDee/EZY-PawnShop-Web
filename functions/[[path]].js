...
export async function onRequest(context) {
  ...
  // ==================== 0. REAL-TIME CLOUD LOGIN ENDPOINT ====================
  if (url.pathname === '/api/login' && request.method === 'POST') {
    try {
      ...
      if (env.DB) {
        // ค้นหาลูกค้าจาก Cloudflare D1 SQL Database โดยตรง
        ...
        if (custRows && custRows.length > 0) {
          ...
          if (customer) {
            // ดึงตั๋วเฉพาะของลูกค้ารายนี้จาก D1
      ...
      return new Response(JSON.stringify({ success: false, message: 'ไม่พบข้อมูลลูกค้าในระบบ หรือเบอร์โทรศัพท์ไม่ถูกต้อง' }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders }
      });
    } catch (err) {
